...
Request-rendered HTML consumes prepared entries and headings. Markdown versions, `llms-full.txt`, reusable-snippet expansion, and syntax highlighting remain build-time work, and their routes must stay prerendered. `llms.txt` files remain indexes of those Markdown versions. Run a production build after changing these options; development mode does not validate the final Worker bundle.
