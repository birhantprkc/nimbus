...
| `icons`              | `true`                  | Built-in icon system. `true` auto-detects `@iconify-json/*` packages and local `src/icons/*.svg`. `false` to disable. Object for explicit config (`iconDir`, `include`, `svgoOptions`). See [Icon](/components/icon). |
