...
The installer wires the adapter and server Wrangler configuration, then adds `rendering: { default: "request" }` when the active Nimbus config has no rendering policy. Existing policies are preserved. If the config is imported or otherwise cannot be edited safely, the installer prints a coding-agent runbook instead:
