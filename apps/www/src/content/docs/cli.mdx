...
Project-owned Nimbus configuration is handed to a coding agent rather than parsed by a brittle codemod. Inside a detected coding agent, the command emits the versioned runbook directly. From a regular shell, use `--print` to pipe it manually:
