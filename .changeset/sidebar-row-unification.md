...
- `ApiBody` renders every request-body media type: each additional media shows a labelled section with its fields or union and an inline highlighted example, and a scalar body (e.g. `application/octet-stream`) keeps its affordance. The primary body badge is media-driven instead of a hardcoded `JSON`.
